...
//static void buildException(uint8_t u8exception, EXCHANGE_DataExchange_TypeDef *dataExchange);
...
const unsigned char fctsupported[] = { MB_FC_READ_COILS, MB_FC_READ_DISCRETE_INPUT, MB_FC_READ_REGISTERS, MB_FC_READ_INPUT_REGISTER, MB_FC_WRITE_COIL, MB_FC_WRITE_REGISTER, MB_FC_WRITE_MULTIPLE_COILS,
...
#else		  
          if (!xTaskCreate(StartTaskModbusMaster, "TaskModbusMaster", 1024, modH, tskIDLE_PRIORITY + 1 , NULL)) {
                    return;
                }
#endif
...
{
    struct netbuf *inbuf;
    err_t recv_err = 0, accept_err = 0;
    char *buf = NULL;
    uint16_t buflen = 0;
    uint16_t uLength = 0;
    bool xTCPvalid = 0;
    xTCPvalid = false;
    tcpclients_t *clientconn = NULL;
    ...
    if (recv_err == ERR_TIMEOUT) //No new data
    {
        ...
        //UpdateTCP_Error(31);
        //UpdateTCP_Aging((unsigned short) clientconn->aging);
    ...
    else if (clientconn->conn != NULL)
    {
        //continue the aging process
        modH->newconns[modH->newconnIndex].aging++;
        //UpdateTCP_Error(recv_err);
        //UpdateTCP_Aging((unsigned short) clientconn->aging);
...
static mb_errot_t TCPgetRxBuffer(modbusHandler_t *modH)
{
    ...
    if (err == ERR_OK)
    {
        ...
        if (err == ERR_OK)
        {
            ...
            if (err == ERR_OK)
            {
                if ((buflen > 11 && (modH->uModbusType == MB_SLAVE)) || (buflen >= 10 && (modH->uModbusType == MB_MASTER))) // minimum frame size for modbus TCP
                {
                    if (buf[2] == 0 || buf[3] == 0) //validate protocol ID
                    {
                        uLength = (buf[4] << 8 & 0xff00) | buf[5];
...
modbusHandler_t* modH = (modbusHandler_t*)argument;
    ...
    for (;;)
    {
        ...
        if (modH->xTypeHW == TCP_HW)
        {
            ...
            ulNotificationValue = TCPconnectserver(modH, &telegram); ///TODO откуда ulNotificationValue???
            if (ulNotificationValue == ERR_OK)
            {
                SendQuery(dataExchange, telegram);
...
uint8_t validateRequest(modbusHandler_t *modH)
{
    ...
    // check fct code
    bool isSupported = false;
    for (uint32_t i = 0; i < sizeof(fctsupported); i++)
    {
        if (fctsupported[i] == modH->u8Buffer[FUNC])
        {
            isSupported = 1;
...
//            if ((u16AdRegs + u16NRegs) > modH->u16regsize) ////< Что за u16regsize?
 ...
 * @ingroup modH modbus handler
 */
//void buildException(uint8_t u8exception, EXCHANGE_DataExchange_TypeDef *dataExchange)
//{
//		modbusHandler_t *modH = &dataExchange->Protocol.ProtocolSettings.ModbusSettings.MbSettings;
...
//            HAL_HalfDuplex_EnableTransmitter(modH->port);
        ...
        else
        {
        	...
        	// memcpy(dataExchange->Interface.TxData, modH->u8Buffer, modH->u8BufferSize);
        	// EXCHANGE_Transmit(dataExchange, modH->u8BufferSize, 0);
...
//        u8MBAPheader[5] = modH->u8BufferSize; //highbyte data length
      	...
      	if (dataExchange->Interface.InterfaceType == EXCHANGE_INTERFACE_ETH1)
      	{
      		memmove(modH->u8Buffer + 6, modH->u8Buffer, modH->u8BufferSize);
      		memcpy(modH->u8Buffer, dataExchange->Interface.InterfaceSettings.ETH1_Settings.ClientServer.clients[index].RxBuffer, 6);
      	}
//      	else
//      	{
//      		memmove(modH->u8Buffer + 6, modH->u8Buffer, modH->u8BufferSize);
//      		memcpy(modH->u8Buffer, dataExchange->Interface.InterfaceSettings.ETH2_Settings.ClientServer.clients[Index].RxBuffer, 6);
//
//      	}
//        xNetVectors[0].len = 6;
...
//        err_enum_t err;
				...
				if (dataExchange->Interface.InterfaceType == EXCHANGE_INTERFACE_ETH1)
				{
					memcpy(dataExchange->Interface.InterfaceSettings.ETH1_Settings.ClientServer.clients[index].TxBuffer, modH->u8Buffer, modH->u8BufferSize + 6);
				}
//				else
//				{
//					memcpy(dataExchange->Interface.InterfaceSettings.ETH2_Settings.ClientServer.clients[Index].TxBuffer, modH->u8Buffer, modH->u8BufferSize + 6);
//				}


      	if (EXCHANGE_Transmit(dataExchange, modH->u8BufferSize + 6, index) == -1)
      	{
          ModbusCloseConnNull(modH);
      	}
...
//    uint16_t u16currentRegister;
    ...
    u8bitsno = 0;
    modH->u8Buffer[modH->u8BufferSize] = 0;
    for (u16currentCoil = u16StartCoil; u16currentCoil < u16StartCoil + u16Coilno; u16currentCoil++)
    {
//        u16cil = u16StartCoil + u16currentCoil;
...
int8_t process_FC3(modbusHandler_t *modH)
{
    ...
    for (unsigned short currentReg = ((u16StartAdd + firstAnalogInputOffset) % FOG_LIGHT_B_MAX_ADDR); 
        ...
        if (Modbus_Regmap_GetCopyOfItem(currentReg, &currentValue, sizeof(uint16_t))) {
            	modH->u8Buffer[modH->u8BufferSize] = highByte(currentReg);
				modH->u8BufferSize++;
				modH->u8Buffer[modH->u8BufferSize] = lowByte(currentReg);
				modH->u8BufferSize++;
        }                    
    }
...
{
    uint8_t u8currentBit = 0;
    uint16_t u16currentRegister = 0;
